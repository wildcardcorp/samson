...
def aes_ctr_oracle(message):
    ciphertext = AES_CTR(gen_rand_key(), gen_rand_key(8)).encrypt(zlib.compress(message))
    return len(ciphertext)


def aes_cbc_oracle(message):
    ciphertext = encrypt_aes_cbc(gen_rand_key(), gen_rand_key(), zlib.compress(message))
    return len(ciphertext)
...
class CompressionRatioSideChannelTestCase(unittest.TestCase):
    ...
    def _execute(self):
        ...
        self.request = lambda msg: aes_cbc_oracle(format_req(msg))
        attack = CompressionRatioSideChannelAttack(self)
