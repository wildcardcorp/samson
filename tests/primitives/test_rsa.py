...
class RSATestCase(unittest.TestCase):
    ...
    def test_openssh_gauntlet(self):
        ...
        for i in range(num_runs):
            ...
            priv        = rsa.export_private_key(encoding='OpenSSH', encryption=b'aes256-ctr', passphrase=passphrase)
            pub_openssh = rsa.export_public_key(encoding='OpenSSH')
            pub_ssh2    = rsa.export_public_key(encoding='SSH2')
            ...
            self.assertTrue((new_priv.d, new_priv.e, new_priv.n, new_priv.p, new_priv.q), (rsa.alt_d, rsa.e, rsa.n, rsa.p, rsa.q))
            self.assertTrue((new_pub_openssh.e, new_pub_openssh.n), (rsa.e, rsa.n))
            self.assertTrue((new_pub_ssh2.e, new_pub_ssh2.n), (rsa.e, rsa.n))
    ...
    def test_jwk_gauntlet(self):
        for _ in range(100):
            ...
            priv = rsa.export_private_key(encoding='JWK')
            pub  = rsa.export_public_key(encoding='JWK')
