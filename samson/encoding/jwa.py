...
class JWA_PS(object):
    ...
    def sign(self, key: object, data: bytes) -> Bytes:
        return key.decrypt(PSS(key.n.bit_length(), mgf=lambda x, y: MGF1(x, y, self.hash_obj), hash_obj=self.hash_obj).pad(data))


    def verify(self, key: object, data: bytes, sig: bytes) -> bool:
        return PSS(key.n.bit_length(), mgf=lambda x, y: MGF1(x, y, self.hash_obj), hash_obj=self.hash_obj).verify(data, key.encrypt(sig))
...
JWA_ALG_MAP = {
    ...
    JWA.ES256: JWA_ES(P256),
    JWA.ES384: JWA_ES(P384),
    JWA.ES512: JWA_ES(P521),
    JWA.RS256: JWA_RS(SHA256()),
    JWA.RS384: JWA_RS(SHA384()),
    JWA.RS512: JWA_RS(SHA512()),
    JWA.PS256: JWA_PS(SHA256()),
    JWA.PS384: JWA_PS(SHA384()),
    JWA.PS512: JWA_PS(SHA512())
}
