...
class GFPolyElement(object):
    def __init__(self, poly):
        if type(poly) is int:
            poly = int_to_poly(poly)

        self.poly = poly
    

    def rem_and_poly(self, result):
        return self.__class__(Poly(gf_rem(result, self.parent.reducing_poly, self.parent.p, ZZ), x, modulus=self.parent.p))
    ...
    def __repr__(self):
        return f"<GFPolyElement: {self.poly}/{Poly(self.parent.reducing_poly, x, modulus=self.parent.p)}>"
...
def GFPolyFactory(_mod, _dom, _parent):
...
class GFPoly(Field, SimpleDomain):
    def __init__(self, p: int, n: int=1, reducing_poly: Poly=None):
        ...
        if reducing_poly:
            self.reducing_poly = reducing_poly.all_coeffs()
        else:
            if n == 1:
                self.reducing_poly = Poly(x, x)
            else:
                for c in itertools.product(range(p), repeat=n):
                    poly = (1, *c)
                    if gf_irreducible_p(poly, p, ZZ):
                        self.reducing_poly = poly
                        break
        

        dom = ZZ
        self.dtype = GFPolyFactory(p, dom, self)
        self.zero  = self.dtype(0)
        self.one   = self.dtype(1)
        self.dom   = dom
        self.mod   = p
        self.poly_as_int = poly_to_int(Poly(self.reducing_poly, x, modulus=self.p))



    def __repr__(self):
        return f"<GFPoly: p={self.p}, n={self.n}, reducing_poly={Poly(self.reducing_poly, x, modulus=self.p)}>"
